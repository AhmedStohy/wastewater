...
def create_hparams():
    params = tf.contrib.training.HParams(
        ...
        savedir='ckpt/trm_200406x0varilen',  # 保存实验结果
        model_select='trainsformer',
        ...
        # hid_dims = [128],
        # out_hid_dims = [256],
        rnn_out_hid_dims=[32, 128, 32],
        rnn_keep_prop=1.0,

        # CNN参数
        feature_map_num=[32, 32],  # 最后一个为全幅卷积特征映射数
        # feature_map_num = [128, 128, 128],
        kernel_size=[3],
        # kernel_size = [4], # 一厂入水指标预测
        # out_hid_dims = [256],
        out_hid_dims=[32],  # 预测MLP各隐层神经元个数，[128,128]表示两个维度128的隐层
...
if not os.path.exists(args.processed_data):
    if not args.isVariableLen:
        ...
        data, label = process_raw_data_variable_len(args.raw_data, args.processed_data, args.liuliangInd + 1,
                                                    args.feat_num, args.label_num, args.seq_len_limit)
else:
    if not args.isVariableLen:
        data_label = np.genfromtxt(args.processed_data, np.float32, delimiter='\t')
        data, label = data_label[:, :args.total_feat_num], data_label[:,
                                                           args.total_feat_num:args.total_feat_num + args.label_num]
    else:
